 ...
 * </ul>
 * 
 * <p>该类使用BouncyCastle库处理PEM格式的密钥文件，支持以下功能：</p>
 * <ol>
 *   <li>从resources/static目录加载PEM格式的密钥文件</li>
 *   <li>生成符合FomoPay API规范的请求签名</li>
 *   <li>验证FomoPay API返回的响应签名</li>
 *   <li>处理HTTP请求和响应</li>
 * </ol>
 */
...
public class FomoPayUtil {
     ...
     * 使用私钥对请求进行签名
     * 
     * <p>该方法用于使用RSA私钥对请求数据进行签名，采用SHA256withRSA算法。</p>
     * 
     * <p>签名过程：</p>
     * <ol>
     *   <li>使用SHA-256算法对原始数据进行哈希</li>
     *   <li>使用RSA私钥对哈希值进行加密</li>
     *   <li>将签名结果转换为小写十六进制字符串</li>
     * </ol>
     * 
     * <p>注意：该方法生成的签名符合FomoPay API规范，可用于支付请求的签名验证。</p>
     * 
     * @param payload 要签名的原始数据，通常为JSON字符串
